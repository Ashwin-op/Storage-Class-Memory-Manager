...
static int exists(struct avl *avl, const char *s) {
    uint64_t count;
...
static int insert(struct avl *avl, const char *s) {
    if (avl_insert(avl, s)) {
...
static int remove_word(struct avl *avl, const char *s) {
    if (avl_remove(avl, s)) {
...
static int load(struct avl *avl, const char *s) {
    char word[256];
    FILE *file;
...
static void list_word(void *arg, const char *word, uint64_t count) {
    UNUSED(arg);

    printf("'%s' x %lu\n", word, (unsigned long) count);
}

static int list(struct avl *avl, const char *s) {
    UNUSED(s);
...
static int info(struct avl *avl, const char *s) {
    UNUSED(s);
...
static int help(struct avl *avl, const char *s) {
    UNUSED(avl);
    UNUSED(s);
...
static int quit(struct avl *avl, const char *s) {
    UNUSED(avl);
    UNUSED(s);
...
static int shell_fnc(void *arg, const char *s) {
    const struct {
        int argc;
        const char *face;
